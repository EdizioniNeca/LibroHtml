...
$(window).load(function() {
    ...
    if (cerca != undefined) {
        ...
        var trovato = true;
        var cont = 0;
        searchPrompt(estratto, false);

        function doHighlight(bodyText, estratto, highlightStartTag, highlightEndTag) {
            // parametri opzionali per colorare
            ...
            var newText = "";
            var i = -1;
            var lcSearchTerm = estratto.toLowerCase();
                ...
                if (i < 0) {
                    ...
                    if (bodyText.lastIndexOf(">", i) >= bodyText.lastIndexOf("<", i)) {
                        ...
                        if (lcBodyText.lastIndexOf("/script>", i) >= lcBodyText.lastIndexOf("<script", i)) {
                            newText += bodyText.substring(0, i) + highlightStartTag + bodyText.substr(i, estratto.length) + highlightEndTag;
                            bodyText = bodyText.substr(i + estratto.length);
        ...
        function highlightSearchTerms(searchText, treatAsPhrase, warnOnFailure, highlightStartTag, highlightEndTag) {
            //console.log("searchText",searchText);//tutto il testo della pagina
        ...
        function searchPrompt(defaultText, treatAsPhrase, textColor, bgColor) {
            ...
            return highlightSearchTerms(estratto, treatAsPhrase, true, highlightStartTag, highlightEndTag);
        }
        //se trovate le parole nella ricerca allora alert
        if (trovato == true) {
            //il cont tiene conto anche all'interno delle foto!
    ...
    function() {
        ...
        _createClass(Blocks, [{
            ...
            value: function loadPrevius() {
                ...
                var bloccoprima;
                //quit = true;
                var t1, t2, t3, t3inDom, b, thisTit, thisSection, T2precedente;
                var titPrec;
                        ...
                        if ($("#" + bloccoPrecedente.id).length <= 0) {
                            ...
                            if (!(titBloccoPrec === undefined) && $("#" + titBloccoPrec.id).length > 0) {
                                ...
                                if (bloccoPrecedente.tipo_sez == "B") {
                                    ...
                                    if (thisB.indice == "2" || thisB.indice == "3") {
                                        ...
                                        if (thisBlock.tipo_sez == "T" && thisBlock.indice == "3") {
                                            ...
                                            if (thisBlock.titolo != bloccoPrecedente.id && bloccoPrecedente.indice != "2") {
                                                ...
                                                bloccoPrecedente = this.getPreviusBlock(bloccoPrecedente);
                                                nextBlock = this.getNextBlock(bloccoPrecedente);
                                                if (nextBlock.tipo_sez == "T" && nextBlock.indice == "2") {
                                                    //   alert("cambio di T2");
                                                    if (T2precedente != undefined) {
                                                        tutto = T2precedente.contenuti + bloccoPrecedente.contenuti;
                                                        $(tutto).insertAfter("#" + T1precedente.id);
                                                    ...
                                                    if (bloccoPrecedente.tipo_sez == "I") {
                                                        ...
                                                        alert("Problema T1,simao in cima, tutto da creare");
                                                        bloccoPrecedente = this.getPreviusBlock(bloccoPrecedente);
                                                        titoloPrecedente = bloccoPrecedente.titolo;
                                                        T1precedente = this.getBlockById(titoloPrecedente);
                                                        while (T1precedente.titolo != "") {
...
function ricerca() {
    ...
    var eccolo, cont = 0;
    if (estratto == "") {
        $("#text_search").text("Inserire parola nella barra di ricerca!");
        return;
    } else {
        ...
        for (var i = 0; i < container.length; i++) {
            ...
            var contenitore = $(contenuti).text();
            var contenitore2 = $(id_cont).text();


            eccolo = contenuti.search(estratto);
            ...
            if (eccolo > -1) {
                ...
                $(link1).removeAttr("href");
                window.location.assign("index.html?id=" + id_cont + "&lis=" + listato + "&tit=yes&search=" + estratto + "&menu=1&pos=" + 0);
                return;
            }
    ...
    if (cerca == "" || cerca == undefined) {
        ...
        for (var j = 0; j < tutti.length; j++) {
            if (tutti[0]) {
                ...
                if (tutti[pos] == undefined) {
                    $(".trovati").text("Fine ricerca");
                    $(".src_next").css("display", "none");
                    return;
                } else {
                    window.location.assign("index.html?id=" + tutti[pos] + "&lis=" + listato + "&tit=yes&search=" + cerca + "&menu=1&pos=" + pos + "&ultimo=" + cont);
                }
                return;
            }
        }
    ...
    if (cerca == "" || cerca == undefined) {
        ...
        for (var i = 0; i < container.length; i++) {
            ...
            if (eccolo > -1) {
                ...
                console.log("contenitore2", contenitore2);
                cont--;
                trovato = true;
                ricerca_attiva = true;
                tutti = tutti.concat(id_cont);
            ...
            for (var j = pos; j < tutti.length; j--) {
                if (tutti[0]) {
                    pos--;
                    if (listato == "AB" || listato == "") {
                        var link1 = document.getElementsByClassName("menuAB");
